...
echo "🔥 Running CUM post-commit hook..."
...
if [ ! -f "$TEX_FILE_PATH" ]; then
    echo "⚠️ Hook Warning: LaTeX log file not found at $TEX_FILE_PATH. Recreating it." >&2
...
EOF_INNER
    if [ $? -ne 0 ]; then
        echo "❌ Hook Error: Failed to recreate LaTeX log file at $TEX_FILE_PATH. Summary will not be added." >&2
        exit 0 # Exit gracefully, don't block commit if recreation fails
    fi
    echo "✅ LaTeX log file recreated at $TEX_FILE_PATH."
fi
...
if [ "$sanitized_diff_len" -gt "$MAX_DIFF_CHARS" ]; then
    ...
    echo "⚠️ Diff was truncated from $sanitized_diff_len bytes to $MAX_DIFF_CHARS characters before sending to LLM."
fi
...
if [ $curl_exit_status -ne 0 ]; then
  ...
  if [ -n "$extracted_text" ]; then
    ...
    echo "❌ Hook Warning: Failed to extract summary from LLM response or response was empty/null." >&2
...
if [ $awk_status -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "❌ Hook Warning: Failed to move temporary TeX file. TeX log not updated." >&2
        rm -f "$TMP_TEX_FILE"
    fi
else
    echo "❌ Hook Warning: Failed to write summary to TeX file using awk (status: $awk_status). TeX log not updated." >&2
    rm -f "$TMP_TEX_FILE"
fi

echo "✅ CUM post-commit hook finished for $commit_short_hash."
exit 0
HOOK_SCRIPT_EOF

if [ $? -ne 0 ]; then
    echo "❌ CRITICAL Error: Failed to write post-commit hook script content to $HOOK_PATH." >&2
    exit 1
fi
echo "✅ Post-commit hook content successfully written to $HOOK_PATH."
...
chmod +x "$HOOK_PATH"
if [ $? -ne 0 ]; then
    echo "❌ CRITICAL Error: Failed to make hook script $HOOK_PATH executable." >&2
    exit 1
fi
echo "✅ Post-commit hook made executable at $HOOK_PATH."
